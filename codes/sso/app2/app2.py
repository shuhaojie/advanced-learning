...
app2.config['MYSQL_DATABASE_PASSWORD'] = '805115148'
app2.config['MYSQL_DATABASE_DB'] = 'oauth2'
...
def index():
    ...
    # 如果没有session_id, 需要跳转到sso来判断，是否在其他系统上已经登录
        ...
        return redirect("http://sso.com/is_login?callback_url=http://app2.com")
...
@app2.get('/sso_login')
def sso_login():
    return redirect('http://sso.com/login?callback_url=http://app2.com')
...
def confirm():
    ...
    # 如果验证通过
    if verify:
        response = make_response(redirect(f"http://app2.com"))
...
def logout():
    session_id = request.cookies.get("sessionId")
    response = make_response(redirect(f'http://sso.com/logout?callback_url=http://app2.com&session_id={session_id}'))
    response.delete_cookie("sessionId")
    return response
